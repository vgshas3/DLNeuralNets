...
# print("cosine similarity(mother, girl) = ", cosine_similarity(mother, girl))
...
# triads_to_try = [('small', 'smaller', 'large'), ('small', 'smaller', 'big'), ('cat', 'dog', 'crocodile')]
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    # ALT FrobNorm: mu_B = np.dot(mu, bias_axis) / np.sum(bias_axis * bias_axis) * bias_axis
    mu_B = (np.dot(mu, bias_axis) / (np.linalg.norm(bias_axis)**2)) * bias_axis
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    # Try L2 if sqrt NaN occurs - norm fn add param 2 instead of default Frobenius
